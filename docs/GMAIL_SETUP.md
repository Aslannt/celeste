...
3. With the OpenAI provider, test natural requests such as `Resume mis correos no leidos`.
4. Ask Celeste to prepare a reply draft and verify it appears as a draft in Gmail.
5. Verify no mail is delivered before the Android/Core confirmation is accepted.
6. Test Cancel first; then perform one intentional confirmation using a harmless test recipient/message.
...
OAuth credentials stay inside Celeste Core. Android receives neither the Google client secret nor the refresh token. Tool audit logs record capability/risk/outcome but intentionally omit email bodies and tool arguments.
